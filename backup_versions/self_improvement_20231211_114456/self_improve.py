...
def main():
    ...
    for i in range(iterations):
        ...
        try:
            logs_collected = collect_logs(os.path.join(os.environ.get('PROJECT_DIRECTORY'), 'self_improvement/log_file.log'), start_token)
            for log in logs_collected:
                messages.append({'role': 'system', 'content': log})
            target_file = get_target_file()
            tasks_content = read_file(os.path.join(os.environ.get('PROJECT_DIRECTORY'), 'self_improvement/tasks.txt'))
            if tasks_content:
